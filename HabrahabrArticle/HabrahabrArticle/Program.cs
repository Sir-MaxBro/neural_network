...
namespace NeuralNetwork
{
    class InputLayer
    {
        ...
        //_trainset = new(double[], double[])[]//да-да, массив кортежей из 2 массивов
        //{
        ...
        private IList<Tuple<double[], double[]>> _trainset = new List<Tuple<double[], double[]>>
        {
        ...
        public IList<Tuple<double[], double[]>> Trainset
        {
        ...
        class Neuron
        {
            ...
            public double Derivativator(double outsignal) => 
                outsignal * (1 - outsignal);//формула производной для текущей функции активации уже выведена в ранее упомянутой книге
            public double Gradientor(double error, double dif, double g_sum) => 
                (_type == NeuronType.Output) ? error * dif : g_sum * dif;//g_sum - это сумма градиентов следующего слоя
        }
        abstract class Layer
        {
            ...
            public double[,] WeightInitialize(MemoryMode mm, string type)
            {
                ...
                XmlDocument memory_doc = new XmlDocument(); // грузим файл с весами
        ...
        {
            //все слои сети
            private InputLayer _input_layer = new InputLayer();
            ...
            static void Train(Network net)//backpropagation method
            {
                ...
                do
                {
                    for (int i = 0; i < net._input_layer.Trainset.Count; ++i)
                    {
                        //прямой проход
                        net.hidden_layer.Data = net._input_layer.Trainset[i].Item1;
                        net.hidden_layer.Recognize(null, net.output_layer);
                        net.output_layer.Recognize(net, null);
                        //вычисление ошибки по итерации
                        double[] errors = new double[net._input_layer.Trainset[i].Item2.Length];
                        for (int x = 0; x < errors.Length; ++x)
                            errors[x] = net._input_layer.Trainset[i].Item2[x] - net.fact[x];
